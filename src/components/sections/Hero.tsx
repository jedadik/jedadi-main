...
export default function Hero() {
  return (
    <main
      id="central"
      className="relative isolate flex min-h-[calc(100vh-80px)] items-center overflow-hidden px-4 py-12 sm:px-8 sm:py-24 lg:px-10 lg:py-28"
      style={{
        backgroundImage:
    ...
    >
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col items-center text-center">
        <div className="mx-auto mt-1 mb-2 flex w-full justify-center sm:mt-2 sm:mb-3">
          <details className="group relative">
            ...
            <div className="absolute left-1/2 top-[calc(100%+0.25rem)] z-30 w-60 -translate-x-1/2 border border-cyber-blue/35 bg-[#080C14]/95 p-3 text-left shadow-[0_0_30px_rgba(19,191,255,0.18)] backdrop-blur-xl">
        ...
        <div className="mt-2 flex items-center gap-3 font-mono text-[10px] uppercase tracking-[0.22em] text-cyber-blue sm:mt-3">
          <span className="h-px w-8 bg-cyber-blue/60 sm:w-14" />
          <span>Central intelligence / 001</span>
          <span className="h-px w-8 bg-cyber-blue/60 sm:w-14" />
        </div>

        <h1 className="max-w-5xl text-balance text-[2.9rem] font-semibold leading-[0.96] tracking-[-0.04em] text-white sm:text-7xl lg:text-8xl">
          Look always
          <span className="block bg-gradient-to-r from-cyber-blue via-white to-neon-purple bg-clip-text text-transparent">ahead.</span>
        </h1>
        <p className="mt-5 max-w-xl text-[0.96rem] leading-7 text-muted-gray sm:mt-8 sm:max-w-2xl sm:text-lg sm:leading-8">
        ...
        <div className="mt-8 flex w-full flex-col items-stretch justify-center gap-3 sm:mt-11 sm:w-auto sm:flex-row sm:items-center">
          <a
            href="#ecosystem"
            className="inline-flex min-h-12 items-center justify-center border border-cyber-blue/45 bg-cyber-blue/5 px-6 py-3 font-mono text-[10px] font-bold uppercase tracking-[0.12em] text-cyber-blue transition active:scale-95 hover:border-cyber-blue hover:bg-cyber-blue/12 hover:shadow-[0_0_24px_rgba(19,191,255,0.14)] sm:min-h-14 sm:text-xs"
          >
            Explorar Ecosistema
          </a>
        </div>

        <div className="mt-12 w-full max-w-4xl border border-white/10 bg-titanium-dark/65 text-left shadow-[0_20px_80px_rgba(0,0,0,0.28)] backdrop-blur-md sm:mt-24">
          ...
          <div className="grid divide-y divide-white/10 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
            {metrics.map((metric) => (
              <div key={metric.label} className="border-b border-slate-800/60 px-4 py-3 last:border-none sm:border-b-0 sm:px-5 sm:py-6">
